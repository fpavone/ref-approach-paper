...
\begin{reply}
See \citet{piironen2020projective}.
\end{reply}
...
\begin{reply}
\fede{There is not need of calibration of the pvalues since they are used only to select the "worse" predictor. Then the variable is excluded or not looking at the predictive performance of the reduced model compared to the not-reduced one. In such a way, the posterior pvalues matter only for their ordering and not for their magnitude.}
\end{reply}
...
\fede{At each iteration, the best model is selected via the default stopping rule implemented the \texttt{R}-package \texttt{projpred}. For details, see \citet{piironen2020projective} for details. In few words, the predictive performance is evaluated via cross-validation both for the reference model and the submodels using the log-predictive density (lpd) score. The obtained lpd samples are used to estimate that probability under normality assumption.}
\end{reply}
...
\begin{reply}
\fede{The above condition comes from the implementation of the projection predictive approach \citet{piironen2020projective} in the \texttt{R}-package \texttt{projpred}. The fact that the algorithm is based on the probability of that condition and not on the condition itself make it not that exigent.}
\end{reply}
...
\begin{reply}
\fede{The choice of $\alpha$ to determine the submodel size is discussed in \citet{piironen2020projective}. One possible standard way to proceed, but not guaranteed to be the best in any case, is to choose $\alpha=0.16$, which correspond to requiring that the submodel predictive score is at most at one standard deviation distance from the best submodel predictive score. For further details see \ref{reply:stopping} and \citet{piironen2020projective}.}
\end{reply}
