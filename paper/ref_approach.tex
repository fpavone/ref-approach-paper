...
In the latter case, where the focus is on prediction, the true data generation
...
regression, the observed empirical data distribution is utilized as a
...
inference prodecure, it can be always seen as acting as a filter on the observed 
...
the false discovery rate, sensitivity and stability of the selection. For fully Bayesian methods, we additionally report results in terms of shrinkage and accuracy of the posterior estimates.
...
observations but a lot of unknown model parameters. These occur frequently, for example, in microarray data analysis in medicine or biology.
