...
In the experiments in the next Sections we include a method which we refer to as iterative lasso. It consists in the same algorithm of the iterative projection, expect for not using any reference model. Given the implementation of projpred in the R-package, it means that the predictions of the reference model consist simply on the observed target values. Iterative lasso is therefore used as the free-reference-model comparison baseline of the iterative projection. 
