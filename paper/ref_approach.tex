...
smallest size which has an elpd score similar to the reference model
...
\citet{paper:projpred} for more details. projpred selection is made
using the \texttt{projpred} package.
...
here the decisive feature explaining the difference in the performance.  The
...
the benefits and limitations of the reference model approach.
...
target, but given the minimal subset these features are nit able to
...
\begin{itemize}
\item projpred: The reference model is a Bayesian linear regression
  model (ie, the posterior predictive distributions are used) using
  the first five supervised principal components. The search heuristic
  is forward search, and the predictive performance is estimated via
  10-fold cross-validation.
\item steplm (data based): Stepwise selection using AIC as in the bodyfat example.
\item steplm using a reference model: The reference model is the same as for projpred, but only point predictions (posterior predictive means) $\hat{y}$ are used to replace the target variable $y$. Same AIC selection is used in data based steplm.
\item Bayesian stepwise selection (data based): At each step the model
  fitted is a Bayesian linear regression using regularised horseshoe
  prior and the feature excluded is the one with higher Bayesian
  $p$-value, defined as:
\begin{equation}
...
\item Bayesian stepwise selection (ref model based): same as the data based, but instead of using the observed data values to fit the models at each step, it uses the observed covariate and the target values predicted by a reference model 
\end{itemize}

In Figure \ref{fig:rmse_vs_fdr} it is shown, for different values of
$n$ and $\rho$, the predictive performance in terms of root mean
square error and the false discovery rate of the selected submodel,
averaged after 100 data simulations. projpred has superior FDR and the
...
measurements of the actual stability. The use of a reference model
...
the other ones. This is likely due to as projpred is based on better
...
\section{Complete feature selection}
