...
\title{Using reference models in feature selection 
  ...
  \author{Federico Pavone, Juho Piironen, Paul-Christian B\"{u}rkner and Aki Vehtari}
    ...
    \author{
    Federico Pavone \footnote{Department of Computer Science, Aalto University, Finland.} $\,$\footnote{Correspondence concerning this article should be addressed to TODO.}, 
  Juho Piironen$^*$,
...
\maketitle
\begin{abstract}
  Feature selection, or more generally, model reduction is an important aspect of the statistical workflow aiming to provide insights from data. In this paper we discuss and demonstrate the benefits of using a reference model in feature selection. A reference model acts as a noise-filter on the target variable by modeling its data generating mechanism. As a result, using the reference model predictions in the model selection procedure reduces the variability and improves stability leading to improved model selection performance. Assuming that a Bayesian reference model describes the true distribution of future data well, the theoretically preferred usage of the reference model is to project its predictive distribution to a reduced model leading to projection predictive feature selection approach. Alternatively, reference models may also be used in an ad-hoc manner in combination with common feature selection methods.  In several numerical experiments, we investigate the performance of the projective prediction approach as well as alternative feature selection methods with and without reference models. Our results indicate that the use of reference models generally translates into better and more stable feature selection. Additionally, we demonstrate that the projection predictive approach shows superior performance as compared to alternative feature selection methods independently of whether or not they use reference models.
...
workflow is feature selection, which is a special case of
model reduction. Feature selection (also known as variable or 
covariate selection) may have multiple goals.  First,
if the features themselves are of interest, we can use feature
selection to infer which features contain predictive information
...
we may be interested in finding the minimal subset of features which
...
model including all features.  The main assumption under any
...
prediction, but whose features are difficult to be understood in
...
models offer a powerful approach to improving feature selection as we
...
feature selection, respectively, before we end with a conclusion in Section \ref{conclusion}.

\section{Reference models in feature selection}
\label{ref-intro}

In this section, we will provide an initial motivation and intuition 
for the use of reference models to improve feature selection methods.
...
into the theoretical reasons why reference models help in feature selection.

\subsection{Bodyfat example: Part 1}
\label{bodyfat-1}

To motivate the further discussion and experiments, we start by simple
feature selection example using body fat data by \citet{johnson1996fitting}.
...
features which are anthropometric measurements (e.g., height, weight
and circumference of different body parts). The
features are highly correlated which causes additional
...
the amount of body fat well while requiring least amount of
...
all the features using a regularised horseshoe prior~\citep{paper:rhs} on the
feature coefficients. Submodels are explored using
...
at 35\%.  The steplm approach has seven features with inclusion
...
sizes of the selected models with projpred are much smaller with
steplm.
\begin{figure}[tp]
  \centering
...
difference in the selected features. This can be explained by high
correlation between the features and different combinations can
provide similar predictive accuracy.

We repeat the experiment with a modified data set by adding 84
unrelated noisy features so that there are 100 feature in total. 
The last two rows of Table
\ref{tab:model_performances} show the cross-validated RMSE, the size
of the selected model and the number of selected noisy features using
projpred or steplm. The results show that projpred has similar
predictive performance and the same number of selected features as
with the original data, whereas the stepwise regression has worse
predictive performance and the number of selected features is much
higher and include a large number of irrelevant features.
...
\caption{Predictive performances with original bodyfat data (first two
  ...
  selected submodel; \# Selected = selected number of features;
  \# Selected noisy = selected number of extra noisy features.}
...
overfitting, but even with 100 features, projpred is able to select a
...
with respect to quality of the feature selection is indeed whether or 
...
Sometimes including all the available features in an
...
obscure the relevance of the features in relation to the target
...
observation. The first $k$ features are strongly related to the
target variable $Y$ and correlated among themselves. Precisely, $\rho$
is the correlation among any pair of the first $k$ features, whereas
$\sqrt{\rho}$ and $\sqrt{\rho/2}$ are the level of correlation between
any relevant feature and, respectively, $f$ and $Y$. If we had an
...
indicator of the true relevance of each feature (represented by the
...
using all the features $\{X_{j}\}_{j=1}^{p}$ taking into account
...
features.  Figure \ref{fig:correlation} illustrates this process in
the form of a scatter plot of (absolute) correlations of the
features with $Y$ against the corresponding correlations with the
...
obtain good predictive performance and to select a smaller number of features. 
When the goal is to select a minimal subset of features,
...
The usual reason for this is that the relevant features
which are not in the minimal subset are highly correlated with features already in
...
and without a reference model, respectively. However, the minimal subset feature
selection of the Bayesian linear regression is still less reliable than projpred.

When the feature selection is repeated with different simulated data
sets, there is some variability in the selected features. We
measure the stability of feature selection by computing the entropy
...
features, and the largest entropy would be observed if the approach
would always selected different sets of features.
...
improves the stability of steplm in feature selection slightly, while it makes
...
features (original plus noisy) is 100, as it was in the last part of
...
irrelevant features included in the final model and the out-of-sample
...
the reference model reduces the number of irrelevant features included
...
feature selection but we will test a simple iterative variant for the complete
feature selection case in the section.
 In addition, we analyse the benefits of the reference
model approach in combination with three other
methods which have been specifically designed for complete feature selection. 
...
We modify the projection predictive approach for complete feature
...
excluding the features selected in the previous
...
\subsection{Alternative complete feature selection methods}
...
% scales better to higher number of features as compared to other
...
%prior in which we select features whose 90\% credible intervals
...
horseshoe prior \Citep{paper:rhs} and selecting those features whose posterior
...
To investigate the performance of the four complete selection approaches,
...
sample correlations between the target and the features using the
...
features $\{X_{j}\}_{j=1}^{p}$, and denote
...
$j$-th feature $\{x_{ij}\}_{i=1}^{n}$. Finally, we refer to the
...
observations $n\in\{50,70,100\}$. The total number of features $p$
and the number of relevant features $k$ are fixed to $p = 1000$ and
$k = 100$, respectively. In general, the lower $\rho$ and $n$, the
more challenging the feature selection is. For this example, \citet{paper:projpred} proposed to
use a reference model which a Bayesian linear regression using the first five
supervised principal components (SPC) as features and imposing an hierarchical prior on
their coefficients:
\begin{equation}
\label{eq:ref_mod}
\begin{aligned}
...
% features to one in order to achieve the highest possible
...
enough information to identify the relevant features. The iterative
...
correlation level are small. Tuning $\alpha$ might lead to improved
...
dataset one more time. As earlier we add noisy uncorrelated features
to the original data to get a total of 100 features.
Since we do not have a ground truth available with regard the original
features of the data, we assume it is reasonable to consider all of
them relevant, at least to some degree. The artificially added
features are naturally irrelevant by construction.
We compute
...
carried over for the complete feature selection with iterative
projpred, and the methods specifically designed for the complete 
feature selection perform better.
...
feature selection methods. Overall, using a reference model leads to
...
strongly recommend using projective predictions for minimal subset
 feature selection if possible and feasible. 
...
feature selection. We tested a simple iterative version of projpred,
with mixed results and the methods specifically designed for complete
feature selection (especially loc.fdr) performed better in our
experiments. It is left for future resarch to develop 
a better projective prediction approach for complete feature
selection problems.
...
features for which we want to obtain a minimally relevant
...
\item Rank the features according to their relevance using some
  ...
  feature. The submodels are then naturally identified by their
...
at $\boldsymbol{\theta}_{*}^{s}$ and conditioning on all the feature
...
prediction workflow and implementation see paper by \citet{paper:projpred}.
