...
In statistical applications, one of the main steps of the modelling
workflow is covariate or feature selection, which is a special case of
model reduction.  Simpler models bring advantages in reduced
measurement costs and improved interpretability. Variable selection
can be also used as an inferential tool: the interest is on knowing
which variables contain predictive information about the target
variable of interest.  The variable selection is most challenging in a
\textit{small n large p} scenario, which is common case, for example,
in microarray data analysis in medicine and biology.
...
The goal of the variable selection can be the minimal subset of which
provides good predictive performance (or balance sparsity with
predictive performance), or the all variables which have some
predictive information about the target variable.  In the predictive
performance based variable selection, the future data generation
mechanism can be modelled either using the observed data or a
reference model.
...
% ones.
The data based approaches, such as the Lasso selection
\cite{tibshirani1996regression} and the stepwise backward/forward
regression the observed empirical data distribution is used as a
proxy of the future data usually in combination with cross-validation
or information criteria to take into account the double use of the
data.
%
The reference model based methods approximate the future data
mechanism using the predictive distribution of a reference model,
which can be, for example, the full-encompassing model. The reference
model approach has been used for long time in the Bayesian framework
since seminal work by \citet{paper:reference_lindley}. For more
historical references see review by \citet{vehtari2012survey} and for
most recent methodological development see paper by
...
The reference models have been used also in non-Bayesian context. For example,
\cite{harrell2015regression} refers to the reference model as a full model that can be thought as the ``gold standard''.
...
relevance. % \cite{faraggi2001understanding} propose to combine the benefits of the neural networks, which become what we call reference models, with the interpretability of regression trees, in order to provide a framework to make inferences about risk groups.
...
the observed response quantities $y$.
...
develop such different procedures.
...
\\
We would like also the reader to note that although we focus on
Bayesian approach, the benefits of the reference model approach can be
obtained also in frequentist settings.
