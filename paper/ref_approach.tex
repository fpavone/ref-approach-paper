...
\begin{figure}[tp]
  ...
  \caption{Root mean square error (RMSE) against false discovery rate in the minimal subset variable selection.}
...
% Many Bayesian approaches for complete variable selection first fit a
...
% scales better to higher number of variables as compared to other
...
% they actually impose on the relevant and irrelevant variables. Ideally,
% the irrelevant variables should be completely shrunk to zero while the
% relevant variables should not be shrunk at all. The amount of shrinkage
...
%prior in which we select variables whose 90\% credible intervals
...
% variables to one in order to achieve the highest possible
...
%     depicted. Relevant and non-relevant variables are displayed in red
%     and in blue respectively. Horizontal dashed lines corresponds to
%     the true value of the mean for the relevant variables.\\}
...
% irrelevant variables. This is quantified in Figure \ref{fig:SESE_SSE}
...
% 100 data simulations. Relevant and non-relevant variables are
...
% points to lie on the diagonal. For the non-relevant variables points
...
% approach. For the relevant variables, the points are always located
...
% distinction of the two clusters of variables (relevant and
...
variable selection perform better.
