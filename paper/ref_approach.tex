...
In statistical applications, one of the main steps of the modelling workflow is covariate or feature selection. From a full Bayesian perspective, when the goal of the analysis is to do prediction, no selection should be carried out and integration over all parameters uncertainties, i.e. posterior distributions, is the recommended procedure. However, a high dimensional parameter space brings different difficulties as computation burden and overfitting. For such reason different feature selection procedures have been proposed both in the frequentist and Bayesian setting (e.g. see \cite{paper:feature_selection}, \cite{paper:vehtari_ojanen} and \cite{paper:model_selection}). Sometimes the task of the analysis is not prediction, but rather inference on which variables are significantly related to the phenomenon of interest. A common case is microarray data in medicine or biology, where for example for a very large number of genes, expression levels are measured for two sets of patients labeled as \textit{positive} or \textit{negative} to some disease (typically a \textit{small n large p} scenario).  The main goal of the analysis is to spot which subset of these genes is significantly related to the disease in order to perform further experiments. A possible way to proceed is to perform hypothesis testing to assess the significance of each gene, based on some modelling assumptions. \cite{book:efron} refers to this as multiple hypothesis testing problem. Such example could be still considered as feature selection, intended this time as the problem of ``identifying \textit{all} features that are predictive about (that is, statistically related to) the target variable'' \citep{paper:projpred}.

Regardless which kind of selection problem we are trying to solve, we consider examples of the reference model approach all those methods that make use of a full-encompassing model, i.e. the reference one, to carry out the selection. The idea of using a reference model is already present in some works from the 60s as \cite{paper:reference_lindley}. It is also the basis for the projection predictive approach \citep{paper:original_proj, paper:projpred}, from which we have taken the hint of using the reference model as a filter (see Section \ref{reference-model-approach}). The reference model, if properly designed, is a less noisy representation of the true data generation mechanism with respect to the observed data distribution. Therefore in a variable selection context it improves the selection process in terms of quality and stability of the selected subset of features, whether the goal is the smallest model with high enough predictive performance or the whole subset of relevant features.

In this paper we try to show such benefits with:

\begin{itemize}
\item a real-world data example comparing the projection predictive approach with the stepwise backward regression selection,
\item a structured comparison in the framework of the normal means estimation problem using different state-of-the-art methods with or not a reference model on top of the procedure, with simulated data and a real-world data example.
\end{itemize}

The paper is structured as follows. In Section \ref{reference-model-approach} we introduce the idea of the reference model, its benefits with examples and how we used it as a filter on our data in a very simple way. In Section \ref{comparison} we use different methods for feature selection comparing the selection results using or not the reference model in the framework of the normal means problem. Finally, in Section \ref{conclusions} we present our conclusions.

\hypertarget{reference-model-approach}{%
\section{Reference model approach to feature selection}\label{reference-model-approach}}

The reference model approach has his roots in  the $\mathcal{M}$-\textit{complete} framework \citep{book:bernardo_smith,paper:vehtari_ojanen} and started to be used in the feature selection context already starting from the 60s with \cite{paper:reference_lindley}. There is not a specific definition of what a reference model is; ideally any full-encompassing model which has a good predictive performance is a candidate reference model. General guidelines on how to devise it are the same about any model in a data analysis context, expect for the fact that no selection is carried out for the reference model. \cite{paper:projpred} showed that through the projection predictive approach the reference model can helps in terms of stability and model size in the selection process. In this Section we aim to give some hints on why this happens and on how we can generally rely on a reference model approach to feature selection with a simple example.
