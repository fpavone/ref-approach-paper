...
model reduction. Feature (variable) selection may have multiple goals.  First,
...
covariates are highly correlated which causes additional
...
unrelated noisy features. The last two rows of Table
...
projpred or steplm.  The projection predictive approach has similar
...
performance significantly. The usual reason for this is that the relevant features
...
sets, there is some variability in selected variables. We
...
\subsection{Body fat example: Part 2}\label{real-world-data}}

Here we repeat the selection of Section \ref{intro-example} via
stepwise backward regression. In this case, the overall number of
covariates (original plus noisy) is 100, as it was in the last part of
Section \ref{intro-example}. We compare results with and without using
...
prediction approach (see results of Section \ref{intro-example}).
\begin{figure}[tp]
  \centering
...
feature selection, but we test here a simple iterative variant for the complete
...
methods which have been originally designed for complete feature selection. 
...
\hypertarget{real-world-data}{%
\subsection{Body fat example: Part 3}\label{real-world-data}}

We conclude our complete selection experiments using the body fat
dataset one more time. As earlier we add noisy uncorrelated covariates
...
covariates of the data, we assume it is reasonable to consider all of
...
instead estimate it from the data. We compare the control of the local
...
projpred, and the methods designed for the complete feature selection
perform better.
...
with mixed results and the methds specifically designed for complete
feature selection (especially loc.fdr) performed better in the
experiments. It is left for future resarch whether it is possible to
design a better projection predictive approach based complete feature
selection method.
