...
The goal of the variable selection can be the minimal subset which
provides good predictive performance (or balance sparsity with
predictive performance), or the entire collection of variables which have some
...
% ones.
In the data based approaches, such as the Lasso selection
\citep{tibshirani1996regression} and the stepwise backward/forward
...
data.
%
The reference model based methods approximate the future data generation
...
the noise present in the data, hence to provide an improved and more
...
\boldsymbol{\theta}_{\perp}^{s} = \underset{\boldsymbol{\theta^{s}}\in\Theta}{\text{argmin}} \frac{1}{n}\sum_{i=1}^{n}\text{KL}\;[p(\tilde{y}_{i}|\boldsymbol{\theta}_{*}^{s})\,||\,p(\tilde{y}_{i}|\boldsymbol{\theta}^{s})] \quad s=1,..,S
\end{equation}
