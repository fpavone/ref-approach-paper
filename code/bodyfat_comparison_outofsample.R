...
for(i in 1:times){
  ...
  for(j in 1:n){
    nc <- 5
    dr <- spca(x[-j,],y[-j], nctot=nc, screenthresh = 0.6, window=p, sup.only = T, verbose = F)
    z <- predict(dr, x[-j,])
    
    # fit the model
    model_data <- list(y=y[-j],X=z,p=ncol(z),n=(n-1),s_max=dr$sdev[1])
    ...
    # Mht
    mufit <- predfun(x[j,])
    yfit[j] <- mean(rnorm(1000,mufit,sigma))
  }
...
save.image(file=paste("bodyfat_outofsample_n",n,".Rdata",sep=""))